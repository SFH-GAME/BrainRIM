...
$allUserResults = selectAll('game_runing_results', ['user_id' => $_SESSION['id']]);

$arrBestUserResultEnemiesPassed = null;
$maxEnemiesPassed = -1; // Устанавливаем начальное значение переменной для сравнения
...
foreach ($allUserResults as $row) {//выбираем самый лучший рещультат
   if ($row['enemiesPassed'] > $maxEnemiesPassed) {
      $maxEnemiesPassed = $row['enemiesPassed'];
      $arrBestUserResultEnemiesPassed = $row;
      $bestUserResultEnemiesPassed = $arrBestUserResultEnemiesPassed['enemiesPassed'];
   }
}
foreach ($allUserResults as $row) {//выбираем самый лучший рещультат
